...
const createSemesterRegistrationIntiDB = async (payload: TSemesterRegistration) =>{
    ...
    const isSemesterRegistrationAllReadyExits = await SemesterRegistration.findOne({academicSemester})

    if(isSemesterRegistrationAllReadyExits){
        throw new  AppError(httpStatus.CONFLICT, 'semester registration already exits')
    }
...
const getAllSemesterREgistrationFromDB =async (query: Record<string,unknown>) => {
    const semesterRegistrationQuery = new QueryBuilder(SemesterRegistration.find().populate('academicSemester'), query).filter().paginate().sort()
...
const updateSemesterRegistrationIntoDB =async (id: string, payload:Partial<TSemesterRegistration>) => {
    ...
    const isSemesterRegistrationAllReadyExits = await SemesterRegistration.findById(id)
    ...
    if(currentSemesterStatus === registrationStatus.ended){
        throw new AppError(httpStatus.BAD_REQUEST,`this semester ${currentSemesterStatus}`)
    }
